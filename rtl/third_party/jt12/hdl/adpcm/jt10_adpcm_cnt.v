...
module jt10_adpcm_cnt(
    ...
    output            sel,
    output            roe_n,
    output     [19:0] lookahead_addr,
    output     [ 4:0] lookahead_bank,
    output            lookahead_roe_n,
    output            decon,
...
wire [20:0] next_addr1 = (clr6 && on6) ? {start6,9'd0} :
    (sumup6 && ~skip6 ? addr6 + 21'd1 : addr6);

// Stage 5 already contains the exact address and enable that will reach the
// external ROM pins two cen edges later.  Exporting this point gives a DDR
// backed implementation roughly 96 system clocks of notice while preserving
// the decoder's original fixed-latency interface and state transitions.
wire [20:0] prefetch_addr = (clr5 && on5) ? {start5,9'd0} :
    (sumup5 && ~skip5 ? addr5 + 21'd1 : addr5);
assign lookahead_addr  = prefetch_addr[20:1];
assign lookahead_bank  = bank5;
assign lookahead_roe_n = ~sumup5;
